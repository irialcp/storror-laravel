document.addEventListener('DOMContentLoaded', function() {
    ...
    if (bottone && imageMenuWrap && overlay) { // Aggiunto controllo per assicurare che gli elementi esistano
        ...
        console.warn("Alcuni elementi per la gestione delle immagini Parkour non sono stati trovati (bottone, menu o overlay).");
    }
...
async function loadCarouselContent() {
    ...
    try {
        ...
        carouselItems.forEach(item => {
            if (item.image) {
                imagesContent += `
                    <a href="${item.product_url || '/shop'}" aria-label="Visita il prodotto">
...
function handleResponsiveCarousel() {
    const container = document.querySelector('.carousel-container.responsive-carousel');
    ...
    if (window.innerWidth <= 1024) {
        ...
        console.log('Stili inline rimossi per permettere al CSS responsive di funzionare');
    }
}
...
async function loadUnsplashImages() {
    ...
    if (!container) {
        console.warn("Element with ID 'parkour_images' not found. Unsplash images won't load.");
        return;
    }
    ...
    try {
        ...
        if (!data || !Array.isArray(data.results)) {
            console.warn("Struttura dati inattesa dalla tua API Laravel (Unsplash):", data);
            container.innerHTML = "<p style='text-align: center; color: #a0a0a0;'>Struttura dati inattesa.</p>";
            return;
        }
        ...
        container.innerHTML = "<p style='color: red;'>Errore nel caricamento delle immagini Unsplash. Riprova più tardi.</p>";
    }
}
