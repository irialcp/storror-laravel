...
function renderProducts(products) {
    ...
    if (!products || products.length === 0) {
        productContainer.innerHTML = "<p style='text-align: center; color: #bbb;'>Nessun prodotto disponibile con i filtri selezionati.</p>";
        return;
    }
    ...
    products.forEach(product => {
        ...
        if (product && product.name && product.price && product.image) {
            ...
            const stockStatus = product.in_stock ? '' : '<span style="color: red; font-size: 0.8em;">(Esaurito)</span>';
            
            htmlContent += `
                <div class="product-card" data-in-stock="${product.in_stock || false}">
...
function filterByStock(inStockOnly) {
    console.log('Filtraggio per stock:', inStockOnly ? 'Solo in stock' : 'Tutti i prodotti');
    ...
    if (inStockOnly) {
        ...
        filteredProducts = allProducts.filter(product => {
            ...
            return product.in_stock === true || 
                   product.in_stock === 1 || 
                   product.in_stock === "1" ||
                   (product.stock && product.stock > 0); // Fallback se usi un campo "stock" numerico
        });
        
        console.log(`Trovati ${filteredProducts.length} prodotti in stock su ${allProducts.length} totali`);
    }
