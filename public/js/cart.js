    ...
    try {
        ...
        document.querySelectorAll('.remove-item-button').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (confirm('Sei sicuro di voler rimuovere completamente questo prodotto dal carrello?')) {
        ...
        document.querySelectorAll('.quantity-input').forEach(input => {
            input.addEventListener('change', function() {
                ...
                if (!isNaN(newQuantity) && newQuantity >= 1) { // La quantità minima è 1
                    updateCartItemQuantity(cartId, newQuantity);
                } else if (newQuantity === 0) { // Se l'utente digita 0, chiedi di rimuovere completamente
                    if (confirm('Impostando la quantità a 0, il prodotto verrà rimosso completamente. Vuoi procedere?')) {
        ...
        document.querySelectorAll('.quantity-minus').forEach(button => {
            button.addEventListener('click', function() {
                const cartId = this.dataset.cartId;
                const inputElement = document.querySelector(`.quantity-input[data-cart-id="${cartId}"]`);
                let currentQuantity = parseInt(inputElement.value);
                if (currentQuantity > 1) { // La quantità minima è 1
                    updateCartItemQuantity(cartId, currentQuantity - 1);
                } else if (currentQuantity === 1) { // Se è 1 e si clicca '-', chiedi di rimuovere
...
async function removeFromCart(cartId) {
    try {
        ...
        if (response.ok && result.success) {
            ...
            const message = result.message || 'Errore sconosciuto durante la rimozione completa dal carrello.';
            alert('Errore: ' + message);
        }
    } catch (error) {
        console.error('Errore durante la rimozione completa dal carrello:', error);
        alert('Si è verificato un errore durante la rimozione completa dal carrello.');
    }
}
...
async function updateCartItemQuantity(cartId, newQuantity) {
    try {
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content'); 
        const response = await fetch('/api/cart/update-quantity', { // Nuova route per aggiornare la quantità
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken 
        ...
        if (response.ok && result.success) {
            ...
            const message = result.message || 'Errore sconosciuto durante l\'aggiornamento della quantità.';
            alert('Errore: ' + message);
